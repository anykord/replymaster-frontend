  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      <aside className="w-64 bg-white border-r">
        <div className="p-4 font-bold">Replymaster</div>
        <nav className="px-2 space-y-1 pb-4">
          {items.map(it => (
            <Link key={it.href} href={it.href} className={`block rounded px-3 py-2 text-sm hover:bg-indigo-50 ${router.pathname===it.href ? 'text-indigo-700 font-medium' : 'text-gray-700'}`}>
              {it.label}
            </Link>
          ))}
        </nav>
